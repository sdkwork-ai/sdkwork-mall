...
Consume federated commerce through `@sdkwork/cloudrouter-app-sdk/domains` / `@sdkwork/cloudrouter-backend-sdk/domains`, `@sdkwork/mall-commerce-service`, and T1 domain SDKs (`@sdkwork/shop-app-sdk`, `@sdkwork/order-app-sdk`, `@sdkwork/payment-app-sdk`, …). No raw HTTP. No local SDK forks. Retired `@sdkwork/commerce-*` and `@sdkwork/cloudrouter-*-domain-transport-sdk` packages must not be reintroduced.
